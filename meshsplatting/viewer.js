...
import {loadModel} from './model-loader.js?v=shcache1';
...
try{
 ...
 const mesh=await loadModel(scene,renderer,camera,$('#progress'));ready=true;drawUntil=performance.now()+10000;status.textContent='MeshSplatting · Office 01';const meta=mesh.userData.officeMeta;$('#details').textContent=`MeshSplatting · ${meta.iteration.toLocaleString()}회 · ${(meta.downloadBytes/1e6).toFixed(2)} MB · 고정 포즈 · 560 학습 / 40 검증`;
