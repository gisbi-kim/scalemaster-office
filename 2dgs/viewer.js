...
import {loadModel} from './model-loader.js?v=final1';
...
try{
 ...
 const mesh=await loadModel(scene,renderer,camera,$('#progress'));ready=true;drawUntil=performance.now()+10000;status.textContent='2D Gaussian Splatting · Office 01';const meta=mesh.userData.officeMeta;$('#details').textContent=`2D Gaussian Splatting · ${meta.iteration.toLocaleString()}회 · ${(meta.downloadBytes/1e6).toFixed(2)} MB · 고정 포즈 · 560 학습 / 40 검증`;
